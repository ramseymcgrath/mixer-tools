...
@test "Create version 30 with old swupd-server" {
  ...
  mixer-build-chroots
  sudo mixer build update --prefix $cachedir/swupd_server/ --config ./builder.conf --format 3 --no-publish --new-swupd > $BATS_TEST_DIRNAME/create_update-30.log
}
...
@test "Create version 40 with format/client bump using new server" {
  ...
  sudo mixer build update --config ./builder.conf --keep-chroots --format 4 --new-swupd > $BATS_TEST_DIRNAME/create_update-40.log
}
